#ts_data - Input data in the time series format
#ts_model - ARIMA/ETS
#model_spec - pdq;lambda values for ARIMA/ ETS;damped specification for exponential smoothing
#cleaned - TRUE/FASE
#stl_decompose - TRUE/FALSE
...
egovs_forecasts <- function(input_data, ts_model, model_spec, cleaned = TRUE, stl_decompose = TRUE, forecast_points,
								start_date = c(2012,1), end_date = NULL){

	#Subsetting data from 2012
	
	if(class(input_data) %in% "xts"){
	data_subset <- window(input_data, start = c("2012-01-01"))
	}
	
	ts_data <- ts(as.numeric(data_subset), start = start_date, frequency = 12)

	if(!is.null(end_date)){
	ts_data <- ts(as.numeric(data_subset), start = start_date, end = end_date, frequency = 12)
	}
	...
	#Validating the data type of the input
	#if(!class(ts_data) %in% c("ts")){
	#	stop("Error: Input data should be a time series object")
	#}
...
#TODO
#Include functions to plot
#See if confidence interval has to be included #If yes, then they too have to be seasonlly adjusted
#
